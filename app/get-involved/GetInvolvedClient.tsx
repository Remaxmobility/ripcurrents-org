...
export default function GetInvolvedClient({ content: c }: Props) {
  const ways = [
    {
      icon: '🙋',
      title: c.involved_way_1_title || 'Volunteer',
      desc:  c.involved_way_1_desc  || 'Join our team of safety ambassadors at Great Lakes beaches this summer. We provide training on rip current identification and how to assist beachgoers.',
      cta:   c.involved_way_1_cta   || 'Apply to Volunteer',
      href:  '#contact-form',
    },
    {
      icon: '📱',
      title: c.involved_way_2_title || 'Spread the Word',
      desc:  c.involved_way_2_desc  || 'Share our posts, use #RipCurrentSafety, and tag friends planning a Great Lakes trip. Every share reaches someone who might not know the danger.',
      cta:   c.involved_way_2_cta   || 'Follow Us',
      href:  'https://www.facebook.com/ripcurrentinfo.org',
    },
    {
      icon: '🎓',
      title: c.involved_way_3_title || 'Book a Presentation',
      desc:  c.involved_way_3_desc  || 'Request a free rip current safety presentation for your school, community centre, municipality, or sports club.',
      cta:   c.involved_way_3_cta   || 'Request a Presentation',
      href:  '/schools',
    },
    {
      icon: '🤝',
      title: c.involved_way_4_title || 'Partner With Us',
      desc:  c.involved_way_4_desc  || 'Municipalities, beach operators, school boards, and safety organisations — partner with us to extend Great Lakes safety education across Ontario.',
      cta:   c.involved_way_4_cta   || 'Explore Partnership',
      href:  '#contact-form',
    },
    {
      icon: '💛',
      title: c.involved_way_5_title || 'Donate',
      desc:  c.involved_way_5_desc  || 'Your donation funds educational materials, volunteer training, school programs, and outreach campaigns that save lives.',
      cta:   c.involved_way_5_cta   || 'Support the Mission',
      href:  '#contact-form',
    },
  ]
  ...
  const waysR = useInView(0.15)
  const form  = useInView(0.15)

  const heroSubtitle = c.involved_hero_subtitle || "Rip currents kill people who didn't know what they were. Help us change that."
      ...
      <main>
        ...
        <section className="py-20 bg-ocean-deep">
          <div ref={why.ref} className="max-w-4xl mx-auto px-6 text-center">
            <div className={`transition-all duration-700 ${why.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
              <p className="text-xl text-ocean-foam/80 font-body leading-relaxed mb-6">
                {c.involved_why_para1 || 'The Rip Current Information Project is a community-powered organisation. Every volunteer hour, every social share, every school booking, and every donation directly supports our ability to reach more Canadians before they reach the water.'}
              </p>
              <p className="text-base text-ocean-foam/60 font-body leading-relaxed">
                {c.involved_why_para2 || 'Most rip current drownings are preventable. We need your help to make prevention the norm at every Great Lakes beach in Ontario.'}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="py-24" style={{ background: 'linear-gradient(180deg, #010D1B 0%, #0A2540 100%)' }}>
          <div ref={waysR.ref} className="max-w-7xl mx-auto px-6">
            <div className={`mb-14 transition-all duration-700 ${waysR.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {ways.map((w, i) => (
                <div
                  key={w.title}
                  className={`glass-card p-7 flex flex-col transition-all duration-700 ${waysR.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
          ...
          <div ref={form.ref} className="max-w-3xl mx-auto px-6">
            ...
            >
              ...
              <p className="text-ocean-foam/70 font-body">
                {c.involved_form_note || "Tell us how you'd like to help — volunteering, partnerships, donations, or bookings. We'll get back to you promptly."}
              </p>
            </div>
