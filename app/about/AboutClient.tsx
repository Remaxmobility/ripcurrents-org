...
export default function AboutClient({ content: c }: Props) {
  const pillars = [
    { icon: '🔬', title: c.about_pillar_1_title || 'Research-Informed', desc: c.about_pillar_1_desc || 'We stay current with the latest science on rip current dynamics specific to the Great Lakes ecosystem.' },
    { icon: '📢', title: c.about_pillar_2_title || 'Public Awareness',  desc: c.about_pillar_2_desc || 'We work with media, community groups, and emergency services to make rip current knowledge accessible to all.' },
    { icon: '🎓', title: c.about_pillar_3_title || 'Education Programs', desc: c.about_pillar_3_desc || 'We deliver evidence-based water safety presentations to schools, families, and community groups across Ontario.' },
    { icon: '🤝', title: c.about_pillar_4_title || 'Partnerships',      desc: c.about_pillar_4_desc || 'We collaborate with local authorities, municipalities, and safety organisations to build a culture of prevention.' },
  ]
  ...
  const missionPara  = c.about_mission_para  || 'The Rip Current Information Project is a Canadian non-profit dedicated to reducing rip current-related fatalities on the Great Lakes through education and community awareness.'
  const missionQuote = c.about_mission_quote || 'To reduce rip current-related fatalities on the Great Lakes by ensuring every beachgoer in Ontario has the knowledge to recognise, avoid, and survive a rip current.'
  const whyMatters   = c.about_why_matters   || 'Rip currents are the leading cause of drowning deaths at Great Lakes beaches. Most victims are strong swimmers who simply didn\'t know what they were dealing with.'
    ...
    <>
      ...
      <main>
        ...
        <section className="relative pt-40 pb-24 bg-ocean-deep overflow-hidden">
          <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 50% at 50% 30%, rgba(0,119,182,0.12) 0%, transparent 70%)' }} />
          <div ref={hero.ref} className="max-w-4xl mx-auto px-6 text-center">
            ...
            <h1 className={`font-display text-6xl md:text-8xl text-white mb-6 transition-all duration-700 ${hero.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '100ms' }}>
              ABOUT THE{' '}
              <span style={{ WebkitTextStroke: '2px #00B4D8', color: 'transparent' }}>PROJECT</span>
            </h1>
            <p className={`text-xl text-ocean-foam/70 font-body leading-relaxed max-w-2xl mx-auto transition-all duration-700 ${hero.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '200ms' }}>
          ...
          <div ref={mission.ref} className="max-w-7xl mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-16 items-start">
              <div>
                ...
                <h2 className={`font-display text-5xl md:text-6xl text-white mb-8 transition-all duration-700 ${mission.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '100ms' }}>
                  WHO WE ARE
                </h2>
                <div className={`space-y-5 transition-all duration-700 ${mission.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '200ms' }}>
                  <p className="text-lg text-ocean-foam/80 font-body leading-relaxed">{missionPara}</p>
              ...
              <div className={`transition-all duration-700 ${mission.visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`} style={{ transitionDelay: '300ms' }}>
                ...
                <div className="mt-6 glass-card p-6 border border-danger-orange/20">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-ocean-foam/70 font-body leading-relaxed">{whyMatters}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <section className="py-24" style={{ background: 'linear-gradient(180deg, #010D1B 0%, #0A2540 100%)' }}>
          <div ref={reality.ref} className="max-w-7xl mx-auto px-6">
            ...
            <div className={`grid md:grid-cols-2 gap-8 transition-all duration-700 ${reality.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '200ms' }}>
        ...
        <section className="py-24 bg-ocean-deep">
          <div ref={approach.ref} className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {pillars.map((p, i) => (
                <div key={p.title} className={`glass-card p-6 transition-all duration-700 ${approach.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: `${i * 100}ms` }}>
            ...
            <div className={`mt-16 text-center transition-all duration-700 ${approach.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: '500ms' }}>
              <Link href="/get-involved" className="inline-block px-8 py-4 rounded-full bg-danger-gradient text-white font-semibold font-body hover:shadow-lg hover:shadow-orange-500/30 hover:scale-105 active:scale-100 transition-all duration-200">
                Join the Mission
              </Link>
              <Link href="/learn" className="ml-4 inline-block px-8 py-4 rounded-full border border-ocean-teal/50 text-ocean-teal font-semibold font-body hover:bg-ocean-teal/10 transition-all duration-200">
